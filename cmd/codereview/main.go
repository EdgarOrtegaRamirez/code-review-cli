...
import (
	...
	crlib "github.com/EdgarOrtegaRamirez/code-review-cli/lib"
...
func runDiff(path string) error {
	...
	if err != nil {
		return fmt.Errorf("cannot read diff: %w", err)
	}
	report := crlib.AnalyzeDiff(string(data))
	outputStr := formatReport(report)
	return writeOutput(outputStr)
}

func formatReport(report crlib.Report) string {
	switch format {
	case "json":
		return crlib.FormatJSON(report)
	case "markdown":
		return crlib.FormatMarkdown(report)
	default:
		return crlib.FormatTable(report)
	}
}
